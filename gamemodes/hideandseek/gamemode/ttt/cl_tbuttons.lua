...
function TBHUD:CacheEnts()
    ...
    for _, ent in ipairs(ents.FindByClass("ttt_traitor_button")) do
        if IsValid(ent) and IsValid(LocalPlayer()) and LocalPlayer():IsActiveTraitor(ent) then
...
function TBHUD:PlayerIsFocused()
    ...
    return IsValid(LocalPlayer()) and IsValid(self.focus_ent) and LocalPlayer():IsActiveTraitor(self.focus_ent)
end
...
-- TODO: use_key should update if the user changes their binds mid-game
    ...
    for k, but in pairs(self.buttons) do
        ...
        if not IsValid(focus_ent) and d > focus_d and (self.focus_stick < CurTime() or but == self.focus_ent) then
            ...
            if (x < focus_range and y < focus_range and
                ...
                if delay < 0 then
                    surface.DrawText("Single use")
                elseif delay == 0 then
                    surface.DrawText("Reusable")
                else
                    local txt = string.format("Reusable after %s seconds", tostring(delay))
                ...
                local txt = string.format("Press %s to activate", use_key)
                surface.DrawText(txt)
