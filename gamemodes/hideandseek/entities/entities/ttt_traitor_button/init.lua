...
function ENT:TraitorUse(ply)
   if not (IsValid(ply) and ply:IsActiveTraitor(self)) then return false end
   if not self:IsUsable() then return false end

   if self:GetPos():Distance(ply:GetPos()) > self:GetUsableRange() then return false end

   print(self:GetKeyValues()["RemoveOnPress"])
   print(self:GetKeyValues()["wait"])
   print()
...
local function TraitorUseCmd(ply, cmd, args)
   ...
   if idx then
	 ...
	 if IsValid(ply) and ply:IsActiveTraitor(ent) then ent:TraitorUse(ply) end
      end
   end
end
